...
( function () {
	...
	var KEY = 'docmed-scheme';
	var root = document.documentElement;
	var strings = window.docmedScheme || { toDark: 'Switch to dark mode', toLight: 'Switch to light mode' };

	function isDark() {
		return root.classList.contains( 'docmed-dark' );
	}
	...
	function apply( dark ) {
		root.classList.toggle( 'docmed-dark', dark );
	...
	function init() {
		...
		// started.
		var buttons = document.querySelectorAll( '.docmed-scheme-toggle .wp-block-button__link' );
		...
		try {
			if ( ! localStorage.getItem( KEY ) && window.matchMedia ) {
				window.matchMedia( '(prefers-color-scheme: dark)' ).addEventListener( 'change', function ( e ) {
					...
					root.classList.toggle( 'docmed-dark', e.matches );
					Array.prototype.forEach.call( buttons, label );
				} );
			}
		} catch ( e ) {}
