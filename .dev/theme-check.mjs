 ...
 * Playground for this mounts the build output as `docmed` and installs the
 * plugin from the directory:
 *
 *   bash .dev/build-zip.sh "$SCRATCH/docmed-build"
 *   npx -y @wp-playground/cli@3.1.54 server --port=9491 --php=8.3 --wp=latest \
 *     --mount-before-install="$SCRATCH/docmed-build/docmed:/wordpress/wp-content/themes/docmed" \
...
const theme = process.env.THEME || 'docmed';
