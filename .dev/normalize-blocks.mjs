...
// DOCMED_PATTERNS says where the patterns actually are in that case.
const patterns = process.env.DOCMED_PATTERNS
	? resolve( process.env.DOCMED_PATTERNS )
	: resolve( here, '../patterns' );

const url = process.env.WP_URL || 'http://127.0.0.1:9491';
...
// `<?php ... ?>` -> `DMPHP0`, and back again after the round trip.
...
function restore( content, snippets ) {
	return content.replace( /DMPHP(\d+)X/g, ( _, i ) => snippets[ Number( i ) ] );
}
