 ...
 *
 *   WP_URL=https://colorlibhub.com/docmed node .dev/button-boundary.mjs
 *   DOCMED_DARK=1 WP_URL=… node .dev/button-boundary.mjs
 *   DOCMED_PATHS=/,/about/ WP_URL=… node .dev/button-boundary.mjs
 *
...
const dark = process.env.DOCMED_DARK === '1';
const paths = ( process.env.DOCMED_PATHS || '/,/departments/,/doctors/,/about/,/appointment/,/contact/,/blog/,/your-first-visit/,/category/physiotherapy/,/?s=clinic,/no-such-page/' ).split( ',' );
...
if ( process.env.DOCMED_PALETTE ) {
	...
	const buttons = await page.evaluate( () => {
		return [ ...document.querySelectorAll( '.wp-block-button__link, .wp-element-button' ) ]
			...
			.filter( ( el ) => el.offsetWidth > 0 && ! el.closest( '.docmed-scheme-toggle' ) )
			.map( ( el, i ) => {
