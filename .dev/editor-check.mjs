 ...
 *
 *   WP_URL=http://127.0.0.1:9491 node .dev/editor-check.mjs
...
const only = process.env.PATTERNS ? process.env.PATTERNS.split( ',' ).map( ( s ) => 'docmed/' + s ) : null;
...
const patterns = await page.evaluate( async () => {
	...
	return all.filter( ( p ) => 0 === p.name.indexOf( 'docmed/' ) ).map( ( p ) => ( { name: p.name, content: p.content } ) );
} );
...
for ( const pattern of patterns ) {
	...
	const result = await page.evaluate( () => {
		...
		const icons = [ ...doc.querySelectorAll( '[class*="docmed-icon--"], .docmed-header__square' ) ];
	...
	checked++;
	const file = join( shots, pattern.name.replace( 'docmed/', '' ) + '.png' );
