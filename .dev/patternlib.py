#!/usr/bin/env python3
"""Block markup helpers for Docmed's pattern generators.
...
def columns(cols, align=None, gap=None, vertical=None, stack_on_mobile=True,
    ...
    if gap == "0":
        data.setdefault("style", {}).setdefault("spacing", {})["blockGap"] = {"top": "0", "left": "0"}
    elif gap:
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
          ...
          padding_y=None, anchor=None, focal=None, preset_gradient=None, inner_layout="constrained"):
    ...
    if preset_gradient:
        ...
        step = "" if dim in (0, 50) else " has-background-dim-%d" % (10 * int(dim / 10 + 0.5))
        span = ('<span aria-hidden="true" class="wp-block-cover__background%s '
                ...
                'has-%s-gradient-background"></span>' % (step, preset_gradient))
    elif gradient:
        ...
        # (0 does get one: `has-background-dim-0`).
        step = "" if dim == 50 else " has-background-dim-%d" % (10 * int(dim / 10 + 0.5))
        span = ('<span aria-hidden="true" class="wp-block-cover__background '
                'has-%s-background-color%s has-background-dim"></span>' % (overlay, step))

    url = theme_image(slug)
    attr_json = attrs(data).replace('"DOCMED_IMAGE"', '"%s"' % url)
