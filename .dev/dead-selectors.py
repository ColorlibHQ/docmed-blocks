#!/usr/bin/env python3
"""
Fail on any `docmed-*` CSS selector that nothing in the theme emits.
...
`'docmed-field--' . esc_attr( $name )` or JavaScript such as
`'docmed-' + name` registers its literal prefix, and any selector starting
with that prefix is treated as live. Without that, `.docmed-field--message`
would be reported, although inc/appointment.php builds it for the message field.
...
TOKEN = re.compile(r"docmed-[a-z0-9_-]+")
SELECTOR = re.compile(r"\.(docmed-[a-z0-9_-]+)")
# A runtime prefix is the docmed- token that ENDS a string literal and is then
# concatenated with a variable. It must not be required to START the literal:
#
#   class="docmed-field docmed-field--' . esc_attr( $name ) . '"
#
# begins at `class="`, so the first version of this pattern — which wanted a
# quote directly before the prefix — missed `docmed-field--` and reported the
# live `.docmed-field--message` as dead. Its own first run caught that.
PHP_PREFIX = re.compile(r"""(docmed-[a-z0-9_-]*)['"]\s*\.\s*(?:[a-z_]+\s*\(\s*)?\$""")
# 'docmed-' + name   /   `docmed-${name}`
JS_PREFIX = re.compile(r"""(docmed-[a-z0-9_-]*)(?:['"]\s*\+|\$\{)""")
...
REGISTERED = re.compile(r"""array\(\s*'(docmed-[a-z0-9-]+)'\s*,\s*__\(""")
...
def main():
    ...
    print("%d docmed-* selectors, %d emitted literally, dynamic prefixes: %s"
