 ...
 * the strip.
 *
 *   WP_URL=... DOCMED_PATHS=/,/about/ node .dev/contrast-rendered.mjs
 *   DOCMED_DARK=1 WP_URL=... node .dev/contrast-rendered.mjs
 *
 * @package Docmed
 */
...
const paths = ( process.env.DOCMED_PATHS || process.env.DOCMED_URL || '/,/departments/,/doctors/,/about/,/appointment/,/contact/,/blog/,/your-first-visit/,/category/physiotherapy/,/?s=clinic,/no-such-page/' ).split( ',' ).filter( Boolean );
...
if ( process.env.DOCMED_PALETTE ) {
	await page.addStyleTag( { content: await paletteCss( process.env.DOCMED_PALETTE ) } );
	await page.waitForTimeout( 200 );
}

// DOCMED_DARK=1 measures the same page with dark mode on, which is where the
// palette is lifted rather than replaced and is the likeliest place for a
// pairing to fall under AA.
if ( process.env.DOCMED_DARK ) {
	await page.evaluate( () => {
		document.documentElement.classList.add( 'docmed-dark' );
	} );
...
console.log( `\n${ paths.length } pages, ${ measuredTotal } text nodes, all at AA${ process.env.DOCMED_DARK ? ' (dark mode)' : '' }` );
