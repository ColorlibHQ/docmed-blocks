 ...
 * appointment form inline would freeze its output into the page permanently.
 * `[docmed_form]` survives the round trip because a shortcode is
...
const DOCMED_SETUP_FLAG = 'docmed_front_page_created';
 ...
 */
function docmed_starter_pages() {
	return array(
		'home'        => array(
			'title'    => __( 'Home', 'docmed' ),
			'pattern'  => 'docmed/page-home',
			'template' => 'page-no-title',
		),
		'departments' => array(
			'title'    => __( 'Departments', 'docmed' ),
			'pattern'  => 'docmed/page-departments',
			'template' => '',
		),
		'doctors'     => array(
			'title'    => __( 'Doctors', 'docmed' ),
			'pattern'  => 'docmed/page-doctors',
			'template' => '',
		),
		'about'       => array(
			'title'    => __( 'About', 'docmed' ),
			'pattern'  => 'docmed/page-about',
			'template' => '',
		),
		'blog'        => array(
			'title'    => __( 'Blog', 'docmed' ),
			'pattern'  => '',
			'template' => '',
		),
		'contact'     => array(
			'title'    => __( 'Contact', 'docmed' ),
			'pattern'  => 'docmed/page-contact',
			'template' => '',
		),
		'appointment' => array(
			'title'    => __( 'Appointment', 'docmed' ),
			'pattern'  => 'docmed/page-appointment',
			'template' => '',
		),
	);
}
 ...
 * overwrite the first.
 */
function docmed_create_front_page() {
	if ( get_option( DOCMED_SETUP_FLAG ) ) {
		return;
	}

	// Only on a site that has not been built yet: someone activating Docmed
		...
		array(
			...
			'exclude'        => array_filter(
				array(
					...
					docmed_page_exists( 'sample-page' ),
				)
			),
		)
	);
	...
	if ( count( $existing ) > 1 ) {
		update_option( DOCMED_SETUP_FLAG, 'skipped: site already had pages', false );
		return;
	}
	...
	foreach ( docmed_starter_pages() as $slug => $page ) {
		if ( docmed_page_exists( $slug ) ) {
			continue;
		}

		$content = '';
		if ( $page['pattern'] ) {
			$content = docmed_pattern_content( $page['pattern'] );
			if ( '' === $content ) {
				continue;
			}
		}
	...
	docmed_create_primary_menu( $created );
	...
	if ( $created ) {
		update_option( DOCMED_SETUP_FLAG, gmdate( 'c' ), false );
	}
}
add_action( 'after_switch_theme', 'docmed_create_front_page' );
 ...
 * @return int
 */
function docmed_page_exists( $slug ) {
	$ids = get_posts(
		array(
			'name'           => $slug,
			'post_type'      => 'page',
 ...
 */
function docmed_create_front_page_retry() {
	if ( get_option( DOCMED_SETUP_FLAG ) || ! current_user_can( 'edit_theme_options' ) ) {
		return;
	}

	docmed_create_front_page();
}
add_action( 'admin_init', 'docmed_create_front_page_retry' );
...
function docmed_pattern_content( $name, $seen = array() ) {
	...
	return (string) preg_replace_callback(
		...
		static function ( $matches ) use ( $seen ) {
			...
			$nested = docmed_pattern_content( $attributes['slug'], $seen );
 ...
 */
function docmed_create_primary_menu( $pages ) {
	if ( ! $pages ) {
		return;
	}

	$order = array( 'home', 'departments', 'doctors', 'about', 'blog', 'contact' );
	$items = '';
