 ...
 * call our filter.
 */
const DOCMED_UPDATE_HOST     = 'updates.colorlib.com';
const DOCMED_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/docmed.json';
const DOCMED_UPDATE_CACHE    = 'docmed_update_response';
 ...
 * @return bool
 */
function docmed_updates_enabled() {
	/**
	 * Filters whether Docmed checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'docmed_check_for_updates', true );
}
 ...
 * @return array
 */
function docmed_update_payload() {
	$payload = array(
		'theme'      => 'docmed',
		'version'    => DOCMED_VERSION,
	 ...
	 */
	return apply_filters( 'docmed_update_payload', $payload );
}
 ...
 * @return array|null
 */
function docmed_fetch_update() {
	if ( ! docmed_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( DOCMED_UPDATE_CACHE );
	...
	$response = wp_remote_get(
		add_query_arg( docmed_update_payload(), DOCMED_UPDATE_ENDPOINT ),
		array(
			'timeout'    => 8,
			// Not WordPress's default user agent, which carries the site's
			// address: the hash above is the only identifier sent.
			'user-agent' => docmed_update_user_agent(),
		)
	);

	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( DOCMED_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( DOCMED_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function docmed_check_update( $update, $theme_data, $theme_stylesheet ) {
	// Recognised by its Update URI rather than its directory name: every
	// Colorlib theme shares this host, and a site may install Docmed under
	// another directory (the colorlibhub demo uses themes/docmed-blocks).
	if ( $update || empty( $theme_data['UpdateURI'] ) || DOCMED_UPDATE_ENDPOINT !== $theme_data['UpdateURI'] ) {
		return $update;
	}

	$release = docmed_fetch_update();

	if ( ! $release || version_compare( $release['version'], DOCMED_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => DOCMED_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/docmed/',
...
add_filter( 'update_themes_' . DOCMED_UPDATE_HOST, 'docmed_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function docmed_flush_update_cache() {
	delete_site_transient( DOCMED_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'docmed_flush_update_cache' );
add_action( 'after_switch_theme', 'docmed_flush_update_cache' );
 ...
 * their server.
 */
function docmed_updates_notice() {
	$screen = get_current_screen();

	// Appearance → Themes, where the theme is chosen: Docmed has no settings
	...
	$payload = docmed_update_payload();

	// Docmed ships no companion plugin, so this speaks only for the theme. If one
	...
	$note = __( 'Docmed checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'docmed' );
	?>
	<p class="description docmed-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( DOCMED_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>docmed_check_for_updates</code>'
		);
		?>
	</p>
	<style>.docmed-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
add_action( 'admin_footer', 'docmed_updates_notice' );
 ...
 * @return string
 */
function docmed_update_user_agent() {
	return 'Docmed/' . DOCMED_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function docmed_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = docmed_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'docmed_update_request_args', 10, 2 );
